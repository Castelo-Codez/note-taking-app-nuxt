...
const schema = toTypedSchema(
  z.object({
    ...
    password: z
      ...
      .min(7, "password min is 7 characters"),
  })
);
...
const onSubmit = handleSubmit(async (values) => {
  ...
  isLoadingCred.value = true;
  await signIn("credentials", values);
  isLoadingCred.value = false;
});
function $delEmailServerErrors() {
  isEmailNotExist.value = false;
}
function $delPassServerErrors() {
  passwordIsIncorrect.value = false;
}
onMounted(() => {
  if (route.query.error == "Email Doesn't Exists") {
...
<template>
  <form class="flex flex-col gap-y-5" @submit.prevent="onSubmit">
    <div aria-label="email Input">
      ...
      <input
        v-model="email"
        v-bind="emailAttr"
        @change="$delEmailServerErrors"
        @focusin="$delEmailServerErrors"
        autocomplete="off"
        type="email"
        id="email"
        name="email"
      ...
      />
      <MainErrorMsg :error-msg="errors.email" v-if="errors.email" />
      <MainErrorMsg
    ...
    <div aria-label="password Input" class="relative mb-5">
      <label for="password" class="text-text dark:text-text-dark block mb-3">
        Password
      </label>
      <input
        v-model="password"
        v-bind="passwordAttr"
        @input="$delPassServerErrors"
        @focusin="$delPassServerErrors"
        autocomplete="off"
        name="password"
    ...
    <SubmitButton text="login" class="flex justify-center items-center">
      <LoadingSpinner v-if="isLoadingCred" />
    </SubmitButton>
  ...
  >
    ...
    <LoadingSpinner v-if="isLoadingGit" />
  </SubmitButton>
</template>
