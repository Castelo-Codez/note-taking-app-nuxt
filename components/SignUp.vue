...
const onSubmit = handleSubmit(async (body) => {
  if (!errors.value.email && !errors.value.password) {
    ...
    let req = await axios.post("/api/newUser/**", body);
    isLoading.value = false;
    if (req.data.statusCode === 11000) {
      isEmailExists.value = "Email Already Exists";
    }
...
<template>
  ...
  <form class="flex flex-col gap-y-5" @submit.prevent="onSubmit">
    ...
    <SubmitButton text="sign up" class="flex items-center justify-center">
